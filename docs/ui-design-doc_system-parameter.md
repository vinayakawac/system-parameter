...
| Data region | `div` — `relative min-h-[240px] scroll-mt-6 [&_thead]:sticky [&_thead]:top-0` plus `[&_.NbTable]:overflow-visible!` (no inner scroll area: the page is the only scroll container, so the header sticks to the viewport top; NbTable has no sticky prop and its own container scrolls, which would otherwise trap the sticky header) | `NbTable` (`enableHeader`, `headerData` object, `tableData` rows); `NbLoader` (`position="container"`) while saving; `NbEmptyState` when no rows | Data · Feedback |
...
| Sticky action bar (shell substitute) | `div` — `sticky bottom-0 mt-auto flex gap-4 bg-white px-12 py-6 md:justify-between` (no top border) | `NbParagraph` status (`role="status"`); `NbButton` secondary (Discard); `NbButton` primary with `startIcon` (Save) | Action |
...
- **Responsive:** toolbar, pagination and action rows stack below `md`; table columns use percentage widths and the page has a single scroll bar.
